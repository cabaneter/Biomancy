	...
	private static class AcidEffectHandler implements OpenEndedPipe.IEffectHandler {
		...
		public void applyEffects(OpenEndedPipe openEndedPipe, FluidStack fluidStack) {
			...
			if (level.getGameTime() % 5 == 0) {
				List<LivingEntity> mobs = level.getEntitiesOfClass(LivingEntity.class, openEndedPipe.getAOE(), livingEntity -> !AcidHelper.hasAcidEffect(livingEntity));
				for (LivingEntity mob : mobs) {
					AcidHelper.applyAcidEffect(mob, 4);
				}
